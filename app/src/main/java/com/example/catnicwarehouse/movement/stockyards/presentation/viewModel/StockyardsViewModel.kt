package com.example.catnicwarehouse.movement.stockyards.presentation.viewModel
...
class StockyardsViewModel @Inject constructor(
    ...
    fun onEvent(event: StockyardEvent) {
        when (event) {
            is StockyardEvent.GetWarehouseStockyardsByWarehouseCode -> getWarehouseStockyardsByWarehouseCode(event.warehouseCode)
            StockyardEvent.Reset -> _stockyardsFlow.value = StockyardsViewState.Empty
            is StockyardEvent.GetWarehouseStockyardById -> getWarehouseStockyardById(event.id)
        }
    }
            ...
            getWarehouseStockyardByIdUseCase.invoke(id = id)
                .onEach { result ->
                    when (result) {
                        ...
                        is Resource.Success -> {
                            _stockyardsFlow.value =
                                StockyardsViewState.WarehouseStockByIdFound(warehouseStockyard = result.data)
                        }
